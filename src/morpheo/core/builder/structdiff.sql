...
INSERT INTO buf1(OGC_FID, ACCES, WAY_ID, GEOMETRY) 
SELECT OGC_FID, ACCES, WAY_ID, ST_Buffer(GEOMETRY, $buffersize) FROM edges1
;
...
INSERT INTO buf2(OGC_FID, ACCES, WAY_ID, GEOMETRY) 
SELECT OGC_FID, ACCES, WAY_ID, ST_Buffer(GEOMETRY, $buffersize) FROM edges2
;

-- Compute paired edges

INSERT INTO paired(EDGE1,EDGE2,WAY1,WAY2,DIFF)
SELECT e1.OGC_FID,e2.OGC_FID,e1.WAY_ID,e2.WAY_ID,e2.ACCES - e1.ACCES FROM buf1 AS e1, buf2 AS e2 
WHERE ST_Within(e1.GEOMETRY,e2.GEOMETRY) AND ST_Within(e2.GEOMETRY,e1.GEOMETRY)
AND e1.WAY_ID IS NOT NULL
AND e2.WAY_ID IS NOT NULL
AND e1.ROWID IN (
...
SELECT OGC_FID,WAY_ID, LENGTH FROM edges1 
WHERE OGC_FID NOT IN (SELECT EDGE1 FROM paired) AND WAY_ID IS NOT NULL
;

INSERT INTO added(OGC_FID, WAY, LENGTH)
SELECT OGC_FID,WAY_ID, LENGTH FROM edges2
WHERE OGC_FID NOT IN (SELECT EDGE2 FROM paired) AND WAY_ID IS NOT NULL
;
